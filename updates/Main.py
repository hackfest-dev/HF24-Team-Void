...
class MainWindow(tk.Tk):
    ...
    def on_click1(self):
        ...
        while True:
            ...
            outimg = pc.get_pose_key_angles(img)
            
            cv2.imshow("Live Camera Test", outimg)
            c = cv2.waitKey(1)
            if c == 27:
                break
        cap.release()
    ...
    def on_click2(self):
        ...
        while True:
            if is_playing:
                success, img = cap.read()
                if success:
                    img = pc.get_pose_key_angles(img)

                    if prev_frame is not None:
                        interpolated_frames = int(fps_conversion_factor)
                        for _ in range(interpolated_frames):
                            interpolated_frame = cv2.addWeighted(prev_frame, 0.3, img, 0.7, 0)
